...
class Drones(object):
    def __init__(self, pos, view_range, idx):
        self.pos = pos
...
class EnvDrones(object):
    def __init__(self, map_size, drone_num, view_range, tree_num, human_num, max_steps=1_000):
        ...
        for i in range(drone_num):
            temp_drone = Drones(self.start_pos, view_range, i)
    ...
    def get_drone_obs(self, drone):
        ...
        # get reward
        drone.reward = -1 * (self.human_num - seen_person)
    ...
    def get_reward(self):
        rew = -1 * np.sum([human.is_hidden for human in self.human_list])
    ...
    def reset(self):
        self.__init__(self.map_size, self.drone_num, self.view_range, self.tree_num, self.human_num)
    ...
    def generate_trajectory_human(self):
        human_act_list = []
        for i in range(self.human_num):
            if len(self.human_act_traj[i]) == 0:
                length = np.random.randint(1, 6)
                a = np.random.randint(0, 5)
                self.human_act_traj[i] = [a] * length

            human_act_list.append(self.human_act_traj[i].pop())

        return human_act_list

    def step(self, drone_act_list):
        human_act_list = self.generate_trajectory_human()
        ...
        if self.step_env == self.max_steps:
            done = 1
        else:
            done = 0
